      ...
      <div className="space-y-12">
        ...
        <AnimatePresence mode="wait">
          {showHero && (
            <motion.section
              ...
              className="relative rounded-[2.5rem] overflow-hidden shadow-2xl bg-zinc-950 p-8 pt-12 text-center flex flex-col items-center"
            >
            ...
            {CATEGORIES.map((cat, i) => (
              ...
              >
                <Link href={`/search?q=${cat.slug}`} className="block">
                  <div className="bg-card/40 backdrop-blur-sm border border-border/50 rounded-3xl p-6 flex flex-col items-center gap-3 text-center transition-all active:scale-95 shadow-sm">
                    <div className={cn("p-4 rounded-2xl shadow-inner", cat.bg, cat.color)}>
                      <cat.icon className="h-6 w-6" />
                    </div>
                    <span className="font-black text-[10px] tracking-widest uppercase opacity-80">{cat.name}</span>
                  </div>
                </Link>
              </motion.div>
            ))}
        ...
        {!isProfileLoading && userProfile?.role === 'pembaca' && (
          <motion.section 
            ...
            viewport={{ once: true }}
            className="relative bg-zinc-900 border border-white/10 rounded-[2.5rem] p-8 text-center space-y-6 overflow-hidden shadow-xl"
          >
            ...
            <div className="relative z-10 space-y-6">
                ...
                <Button size="lg" className="rounded-full w-full h-14 font-black text-sm uppercase tracking-widest bg-primary text-white" asChild>
